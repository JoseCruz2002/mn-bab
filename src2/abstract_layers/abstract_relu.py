...
from src2.abstract_domains.ai_util import AbstractElement
from src2.abstract_layers.abstract_conv2d import Conv2d
from src2.abstract_layers.abstract_module import AbstractModule
from src2.mn_bab_shape import AffineForm, MN_BaB_Shape
from src2.state.tags import (
    LayerTag,
    ParameterTag,
    key_alpha_relu,
    key_beta,
...
from src2.utilities.layer_types import is_layer_of_type
...
from src2.utilities.prima_interface import ActivationType, get_prima_constraints
...
class ReLU(nn.ReLU, AbstractModule):
    ...
    def backsubstitute(
        ...
        if prima_constraints_available:
            ...
            #print("-- abstract_relu; backsubstitute; 1")
            #print("prima_output_coefs", torch.isnan(prima_output_coefs).any().item())
            #print("----------------------------------------------------")
            ...
            if abstract_shape.ub is not None:
                ...
                #print("-- abstract_relu; backsubstitute; 2")
                #print("prima_output_coefs", torch.isnan(prima_output_coefs).any().item())
                #print("prima_ub_parameters", torch.isnan(prima_ub_parameters).any().item())
                #print("----------------------------------------------------")
    ...
    def _compute_new_prima_coefficients(
        ...
        #print("-- abstract_relu; _compute_new_prima_coefficients;")
        #print("output_var_coefs", torch.isnan(output_var_coefs).any().item())
        #print("input_var_coefs", torch.isnan(input_var_coefs).any().item())
        #print("const_coefs", torch.isnan(const_coefs).any().item())
        #print("--------------------------------------------------")
        return (
    ...
    def _get_prima_coefficients(  # TODO: move some of this logic into verification_subproblem
        ...
        if layer_tag(self) in prima_coefficients:
            #print("-- abstract_relu; _get_prima_coefficients; 1")
            #print("output_var_coefs[0]", torch.isnan(prima_coefficients[layer_tag(self)][0]).any().item())
            #print("output_var_coefs[1]", torch.isnan(prima_coefficients[layer_tag(self)][1]).any().item())
            #print("output_var_coefs[2]", torch.isnan(prima_coefficients[layer_tag(self)][2]).any().item())
            #print("--------------------------------------------------")
        ...
        #print("-- abstract_relu; _get_prima_coefficients; 2")
        #print("new_prima_coefficients[0]", torch.isnan(new_prima_coefficients[0]).any().item())
        #print("new_prima_coefficients[1]", torch.isnan(new_prima_coefficients[1]).any().item())
        #print("new_prima_coefficients[2]", torch.isnan(new_prima_coefficients[2]).any().item())
        #print("--------------------------------------------------")
    ...
    def _multiply_prima_coefs_and_parameters(  # TODO: move this out
        ...
        # bug here on line 666 -> both prima_coefs and prima_params have Nan
        #print("-- abstract_relu; _multiply_prima_coefs_and_parameters; 11111")
        #print("prima_coefs", torch.isnan(prima_coefs).any().item())
        #print("prima_params", torch.isnan(prima_params).any().item())
        #print("prima_coefs", torch.isinf(prima_coefs).any().item())
        #print("prima_params", torch.isinf(prima_params).any().item())
        #print("--------------------------------------------------")
        temp = prima_coefs.bmm(prima_params)
        #print(temp.shape)
        #print("-- abstract_relu; _multiply_prima_coefs_and_parameters; 222222")
        #print("prima_coefs", torch.isnan(prima_coefs).any().item())
        #print("prima_params", torch.isnan(prima_params).any().item())
        #print("prima_coefs", torch.isinf(prima_coefs).any().item())
        #print("prima_params", torch.isinf(prima_params).any().item())

...
from src2.state.constraints import Constraints, ReadonlyConstraints
from src2.state.layer_bounds import LayerBounds, ReadonlyLayerBounds
from src2.state.parameters import (
    Parameters,
    ParametersForQuery,
    ReadonlyParameters,
    ReadonlyParametersForQuery,
)
from src2.state.prima_constraints import PrimaConstraints, ReadonlyPrimaConstraints
from src2.state.split_state import ReadonlySplitState, SplitState
from src2.state.subproblem_state import ReadonlySubproblemState, SubproblemState
from src2.state.tags import LayerTag, ParameterTag, QueryTag
from src2.verification_subproblem import (
...
def _batch_parameter_dicts(  # TODO: move some of this logic into src2.state.parameters
...
def _unbatch_prima_coefficient_dicts(
    ...
    print(f"-- batching; _unbatch_prima_coefficients_dicts; len(prima_coefficients_set): {len(prima_coefficients_set)}")
    print(f"-- batching; _unbatch_prima_coefficients_dicts; batch_size: {batch_size}")
    for layer_id in prima_coefficients_batch:
        for i, prima_coefficients in enumerate(prima_coefficients_set):
            print("-----------------------")
            print(f"-- batching; _unbatch_prima_coefficients_dicts; prima_coefficients_batch: {prima_coefficients_batch[layer_id]}")
            print(f"-- batching; _unbatch_prima_coefficients_dicts; layer_id: {layer_id}")
            print(f"-- batching; _unbatch_prima_coefficients_dicts; i: {i}")
            print("-----------------------")
